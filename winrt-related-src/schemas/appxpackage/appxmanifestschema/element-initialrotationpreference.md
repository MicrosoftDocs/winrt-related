...
When your app is initially launched, Windows receives its rotation preferences. These preferences remain in place until that app is terminated, with one important exception: if a call is made to [**Windows.Graphics.Display.DisplayProperties.autoRotationPreferences**](/uwp/api/Windows.Graphics.Display.DisplayProperties) to change those preferences. That API is used to inform Windows of a change in your app's rotational preferences, and can be called at any time. Note that this API call does not affect the contents of the **InitialRotationPreference** element in the app's manifest file, which remain as you set them in the manifest editor. Therefore, whenever you relaunch your app, it takes the rotation preferences that you originally specified in your app's manifest.
...
3.  The game detects that the user is accessing the map. It calls [**AutoRotationPreferences**](/uwp/api/Windows.Graphics.Display.DisplayProperties) to tell Windows to change the app's rotational preferences to Portrait and/or PortraitFlipped.
...
7.  The game detects that the user is closing the map page. It calls [**AutoRotationPreferences**](/uwp/api/Windows.Graphics.Display.DisplayProperties) to change its rotational preferences back to its original Landscape settings to continue the game.
8.  The user finishes the game and the app is suspended after the user navigates away for some period of time.
9.  The user resumes the app to play another game. Because the app has not been relaunched, the last preferences declared by the [**AutoRotationPreferences**](/uwp/api/Windows.Graphics.Display.DisplayProperties) API still apply.
...
The key point to note is that once you've called [**AutoRotationPreferences**](/uwp/api/Windows.Graphics.Display.DisplayProperties), it is your responsibility to handle the rotational preferences through that API until the game is terminated (not merely suspended).
...
[**Windows.Graphics.Display.DisplayProperties.AutoRotationPreferences**](/uwp/api/Windows.Graphics.Display.DisplayProperties)
