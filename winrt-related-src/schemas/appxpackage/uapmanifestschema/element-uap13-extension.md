...
| **ResourceGroup** | An optional tag used to group extension activations together for resource management purposes (for example, CPU and memory). See the **Remarks** section in [Application](element-application).md). | An alphanumeric string between 1 and 255 characters in length. Must begin with a letter. | No |  |
