...
no-loc: [Package, Applications, Application, Extensions, uap8:Extension, uap8:AppExecutionAlias, uap8:ExecutionAlias]
---
...
<uap8:ExecutionAlias
  ...
  AllowOverride = 'An optional boolean value.'
...
| **AllowOverride** | A value that indicates whether to override the UWP app executable. | An optional boolean value. | No |  |
| **desktop10:UseDesktopChangeRouter** | Used by debugger applications to avoid file dialog deadlocks when debugging the Windows Explorer process. Only supported on extension instances that specify CompatMode="classic". Only supported on desktop SKUs. | An optional boolean value. | No |  |
