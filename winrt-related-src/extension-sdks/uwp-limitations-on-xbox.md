...
| File system access                  | The **broadFileSystemAccess** capability is not supported in apps and games on Xbox. On PCs, this capability allows apps to get the same access to the file system as the user who is currently running the app without any additional prompts during runtime. For more information about this capability, see [this article](/windows/uwp/files/file-access-permissions#accessing-additional-locations). |
